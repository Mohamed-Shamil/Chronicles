...
const UserContents = (props) => {
  ...
  return (
    ...
    <div className="flex justify-center">
      <div className="pt-6 w-full  pb-12 bg-sideclr mb-10 ">
        <div id="card">
          <h2 className="text-center font-serif uppercase text-2xl xl:text-3xl">
            Recent Articles
          </h2>
          {/* container for all cards */}
          <div className="container min-h-max w-full  lg:w-4/5 mx-auto flex flex-col">
            {posts.map((post, index) => (
              <div
                key={index}
                onClick={() => handleBlog(post._doc._id)}
                className="flex flex-col md:flex-row  bg-white rounded-lg shadow-xl mt-4 w-100 mx-2"
              >
