...
export class BlogHelpers {
  ...
  async editPost(post: object) {
    try {
      const editedPost = await editBlogFromDb(post);
      return editedPost;
    } catch (err) {
      throw { err };
    }
  }
  async blogReportHelper (reportDetails: any) {
    
    try {
      const response = await reportBlogService(reportDetails)
       return response
    }
    catch(err){
      throw err
